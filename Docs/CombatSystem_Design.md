...
핵심 원칙: 모든 공격은 합산 방식
→ 매치-3 장르의 직관적 플레이 (블록 모으기 → 한 방에 터뜨리기)
```
...
**예시 3: 복합 타겟 연결**
```
연결 순서: 소드(전방) → 활(후방) → 대검(전방) → 활(후방)

그룹핑 결과:
그룹 1: [소드 + 대검] (전방, 첫 등장: 1번)
그룹 2: [활 + 활] (후방, 첫 등장: 2번)
...
전략적 의미:
→ 저체력 적이 여럿 있을 때는 작은 공격 여러 번이 유리
→ 과잉 데미지를 고려한 블록 연결이 중요
```
...
모든 블록은 **합산 공격 방식**을 사용합니다.
연결된 같은 타겟의 블록들이 데미지를 합산하여 한 번에 공격합니다.
